...
class ConditionBuilderControl(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "condition-builder-control")
    ...
    def pickerIcon(self, pickerIcon):
        # 非内嵌模式时 弹窗触发icon
        # Icon 图标渲染器 文档：https://aisuda.bce.baidu.com/amis/zh-CN/components/icon
        return self.set("pickerIcon", pickerIcon)

        
    def funcs(self, funcs):
        # 函数集合
        
        return self.set("funcs", funcs)
    ...
    def builderMode(self, builderMode):
        # 展现模式# 可选项: ['simple', 'full']
        
        return self.set("builderMode", builderMode)
    ...
    def addBtnVisibleOn(self, addBtnVisibleOn):
        
        
        return self.set("addBtnVisibleOn", addBtnVisibleOn)

        
    def addGroupBtnVisibleOn(self, addGroupBtnVisibleOn):
        # 表达式：控制按钮“添加条件组”的显示
        
        return self.set("addGroupBtnVisibleOn", addGroupBtnVisibleOn)
    ...
    def formulaForIf(self, formulaForIf):
        # if 里面公式编辑器配置
        
        return self.set("formulaForIf", formulaForIf)
