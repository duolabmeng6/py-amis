...
class DialogAction(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "dialog-action")
    ...
    def dialog(self, dialog):
        ...
        return self.set("dialog", dialog)

        
    def nextCondition(self, nextCondition):
        # 是否有下一个的表达式，正常可以不用配置，如果想要刷掉某些数据可以配置这个。
        # 表达式，语法 `data.xxx > 5`。
        return self.set("nextCondition", nextCondition)
