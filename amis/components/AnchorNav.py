...
class AnchorNav(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "anchor-nav")
    ...
    def linkClassName(self, linkClassName):
        # 导航样式名
        # css类名，配置字符串，或者对象。      className: "red"  用对象配置时意味着你能跟表达式一起搭配使用，如：      className: {         "red": "data.progress > 80",         "blue": "data.progress > 60"     }
        return self.set("linkClassName", linkClassName)

        
    def sectionClassName(self, sectionClassName):
        # 楼层样式名
        # css类名，配置字符串，或者对象。      className: "red"  用对象配置时意味着你能跟表达式一起搭配使用，如：      className: {         "red": "data.progress > 80",         "blue": "data.progress > 60"     }
        return self.set("sectionClassName", sectionClassName)
