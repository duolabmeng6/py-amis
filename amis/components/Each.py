...
class Each(AmisComponent):
    ...
    def itemKeyName(self, itemKeyName):
        # 用来控制通过什么字段读取成员数据，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下
        
        return self.set("itemKeyName", itemKeyName)

        
    def indexKeyName(self, indexKeyName):
        # 用来控制通过什么字段读取序号，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下
        
        return self.set("indexKeyName", indexKeyName)
