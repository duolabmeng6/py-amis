...
class NumberControl(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "number-control")
    ...
    def unitOptions(self, unitOptions):
        # 单位列表
        
        return self.set("unitOptions", unitOptions)

        
    def big(self, big):
        # 是否是大数，如果是的话输入输出都将是字符串
        
        return self.set("big", big)
    ...
    def displayMode(self, displayMode):
        # 输入框为基础输入框还是加强输入框# 可选项: ['base', 'enhance']
        
        return self.set("displayMode", displayMode)

        
    def showAsPercent(self, showAsPercent):
        # 用来开启百分号的展示形式
        
        return self.set("showAsPercent", showAsPercent)
