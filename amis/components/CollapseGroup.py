...
class CollapseGroup(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "collapse-group")
    ...
    def expandIconPosition(self, expandIconPosition):
        # 设置图标位置# 可选项: ['left', 'right']
        
        return self.set("expandIconPosition", expandIconPosition)
    ...
    def enableFieldSetStyle(self, enableFieldSetStyle):
        # 当Collapse作为Form组件的子元素时，开启该属性后组件样式设置为FieldSet组件的样式，默认开启
        
        return self.set("enableFieldSetStyle", enableFieldSetStyle)
