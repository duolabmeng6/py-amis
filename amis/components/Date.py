...
class Date(AmisComponent):
    ...
    def type(self, type):
        # 指定为日期展示类型# 可选项: ['date', 'datetime', 'time', 'static-date', 'static-datetime', 'static-time']
    ...
    def fromNow(self, fromNow):
        # 显示成相对时间，比如1分钟前
        
        return self.set("fromNow", fromNow)

        
    def updateFrequency(self, updateFrequency):
        # 更新频率， 默认为1分钟
        
        return self.set("updateFrequency", updateFrequency)

        
    def displayTimeZone(self, displayTimeZone):
        # 时区
        
        return self.set("displayTimeZone", displayTimeZone)
