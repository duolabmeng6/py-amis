...
class QRCode(AmisComponent):
    def __init__(self):
        super().__init__()
        self.set("type", "q-r-code")
    ...
    def qrcodeClassName(self, qrcodeClassName):
        # css 类名
        # css类名，配置字符串，或者对象。      className: "red"  用对象配置时意味着你能跟表达式一起搭配使用，如：      className: {         "red": "data.progress > 80",         "blue": "data.progress > 60"     }
        return self.set("qrcodeClassName", qrcodeClassName)

        
    def codeSize(self, codeSize):
        # 二维码的宽高大小，默认 128
        
        return self.set("codeSize", codeSize)

        
    def backgroundColor(self, backgroundColor):
        # 背景色
        
        return self.set("backgroundColor", backgroundColor)

        
    def foregroundColor(self, foregroundColor):
        # 前景色
        
        return self.set("foregroundColor", foregroundColor)
    ...
    def imageSettings(self, imageSettings):
        ...
        return self.set("imageSettings", imageSettings)
